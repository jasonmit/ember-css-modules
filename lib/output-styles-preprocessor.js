...
OutputStylesPreprocessor.prototype.toTree = function(inputNode, inputPath, outputDirectory, options) {
  var outputFile = this.options.outputFile || options.outputPaths[this.owner.belongsToAddon() ? 'addon' : 'app'];
  ...
  return stew.debug(mergeTrees([inputNode, this.dynamicHeaderConcat(concatOptions)]), { name: 'css-modules-toTree' });
};
...
OutputStylesPreprocessor.prototype.dynamicHeaderConcat = function(options) {
  ...
  modulesTree = stew.debug(modulesTree, { name: 'css-modules-modulesTree' });
